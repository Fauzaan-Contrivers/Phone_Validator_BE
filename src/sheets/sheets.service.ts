...
export class SheetsService {
  phoneColumns = ['Phone number', 'Phone', 'Number', 'Telephone', 'Mobile', 'Mobile number', 'Cell', 'Cell Phone'];
  constructor(
  ...
  async updateSheet(id: number, file): Promise<{ error: boolean, message: string }> {
    try {
      ...
      const result = bufferStream.pipe(csvParser()) // Adjust delimiter if needed
        .on('data', (row) => {
          // Extract phone number using regular expression or library
          for (const key in row) {
            // Check if the current key is 'Phone number'
            if (this.phoneColumns.some(column => column.toLowerCase() === key.toLowerCase())) {
        ...
        .on('end', async () => {
          // console.log('Phone numbers extracted:', results);
          const phoneNumbers = results.map(phoneNumber => ({ phone: phoneNumber, user: user })); // Prepare data
          const existingNumbers = await this.sheetsRepository.find();
          const uniquePhoneNumbers = phoneNumbers.filter(newEntry => {
            return !existingNumbers.some(existingEntry => existingEntry.phone === newEntry.phone);
          });
          try {
  ...
  async updateUserSheet(id: number, file): Promise<{ error: boolean, message: string }> {
    try {
      const user = await this.userRepository.findOne({ where: { id } })
      ...
      const inputFilePath = path.join(__dirname, '../../uploadedFiles', file.filename);
      const outputFilePath = path.join(__dirname, '../../uploadedFiles', `cleaned_${file.filename}`,);
      // console.log('filePath :>> ', filePath);
      // Check if the file exists
      if (fs.existsSync(inputFilePath)) {
        console.log('Input file found at:', inputFilePath);

        // Read CSV file contents and write to a new CSV file with an additional "highlight" column
        const rows = [];
        fs.createReadStream(inputFilePath)
          .pipe(csvParser())
          .on('data', (row) => {
            // Assuming 'Phone' is the column name for the phone numbers
            const phoneNumberColumn = Object.keys(row).find((column) =>
              this.phoneColumns.some((phoneColumn) => column.toLowerCase().includes(phoneColumn.toLowerCase()))
            );
            if (phoneNumberColumn) {
              const phoneNumber = row[phoneNumberColumn];

              // Check if the phone number is in the adminRecords array
              const isHighlighted = adminRecords.some((adminRow) => adminRow['phone'] === phoneNumber);
          ...
          .on('end', () => {
            ...
            if (rows.length > 0) {
              ...
              csvWriterStream.writeRecords(rows).then(() => {
              });
            } else {
