...
export class AuthService {
    ...
    async sendEmail(email: string = "da1716848@gmail.com") {
        try {
            const user = await this.userRepository.findOne({ where: { email } })
            if (!user) {
                return {
            ...
            const jwtToken = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '10m' });
            const messageData = {
                ...
                <a href="http://localhost:3001/forget-password/${jwtToken}" target="_blank">
                change password
                </a>`
            };
    ...
    async changePassword(id: string, password: string) {
        try {
            ...
            const user: any = jwt.verify(id, process.env.JWT_SECRET);
            console.log(user);
            if (!user) {
                return {
                    error: true,
    ...
    async createSubAdmin(email: string, name: string, role: string) {
        try {
            ...
            if (role === "admin") {
                const subAdminExists = await this.userRepository.findOne({ where: { email } });
                if (subAdminExists) {
